...
interface Emits {
  ...
  (e: 'image-processed', matrix: string[][], imageDescription: string): void
}

const props = defineProps<Props>()
const emit = defineEmits<Emits>()

const { quantizeImage, matrixToPromptDescription, isProcessing } = useImageQuantizer()
...
const handleConfirm = () => {
  ...
  const description = matrixToPromptDescription(quantizedMatrix.value)
  emit('image-processed', quantizedMatrix.value, description)
  handleClose()
}
  ...
  >
      ...
      <div class="flex items-center justify-between gap-3 w-full">
        <div class="flex items-center gap-3">
          <div class="p-2 bg-indigo-500/10 rounded-lg border border-indigo-500/20">
            <UIcon name="i-heroicons-photo" class="w-6 h-6 text-indigo-400" />
          </div>
          <div>
            <h2 class="text-2xl font-bold bg-gradient-to-r from-indigo-400 via-purple-400 to-indigo-400 bg-clip-text text-transparent">
              Bild hochladen
            </h2>
            <p class="text-sm text-gray-400">Wähle einen Bildausschnitt für die Animation</p>
          </div>
        </div>
        <UButton
    ...
    <template #body>
      <div class="p-6 space-y-6 bg-gray-900 rounded-2xl">
        ...
        <!-- Upload Button -->
        <div v-if="!uploadedImage" class="flex flex-col items-center justify-center py-12 border-2 border-dashed border-gray-700 rounded-lg hover:border-indigo-500/50 transition-colors cursor-pointer" @click="triggerFileInput">
        ...
        <div v-if="uploadedImage && !croppedImage" class="space-y-4">
          ...
          >
            ...
            <!-- Crop Overlay -->
            <div
              class="absolute border-2 border-indigo-400 bg-indigo-400/10 cursor-move"
              :style="{
                left: cropArea.x + 'px',
                top: cropArea.y + 'px',
            ...
            >
              <div class="absolute inset-0 grid grid-cols-3 grid-rows-3">
                <div v-for="i in 9" :key="i" class="border border-indigo-400/30"></div>
              </div>
            </div>
          </div>
          ...
          <div class="bg-indigo-500/5 border border-indigo-500/20 rounded-lg p-4">
            <div class="flex gap-3">
              ...
              <div class="text-sm text-gray-400">
                <p>Das Bild wurde auf 16x16 Pixel reduziert und wird dem AI-Prompt hinzugefügt.</p>
              </div>
            </div>
          </div>
